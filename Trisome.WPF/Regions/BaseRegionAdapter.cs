...
namespace Trisome.WPF.Regions
{
    ...
    public abstract class BaseRegionAdapter<T> : IRegionAdapter where T : class
    {
        /// <summary>
        ...
        /// <summary>
        /// Initializes a new instance of <see cref="BaseRegionAdapter{T}"/>.
        /// </summary>
        /// <param name="regionBehaviorFactory">The factory used to create the region behaviors to attach to the created regions.</param>
        protected BaseRegionAdapter(IRegionBehaviorFactory regionBehaviorFactory)
        {
        ...
        protected virtual void AttachDefaultBehaviors(IRegion region, T regionTarget)
        {
            ...
            var behaviorFactory = RegionBehaviorFactory;
            if (behaviorFactory != null)
            {
                foreach (string behaviorKey in behaviorFactory)
                {
                    if (!region.Behaviors.ContainsKey(behaviorKey))
                    {
                        ...
                        if (regionTarget is DependencyObject dependencyObjectRegionTarget)
                        {
        ...
        static T GetCastedObject(object regionTarget)
        {
            if (regionTarget == null)
                throw new ArgumentNullException(nameof(regionTarget));
        ...
        static void SetObservableRegionOnHostingControl(IRegion region, T regionTarget)
        {
