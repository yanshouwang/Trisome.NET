...
namespace Trisome.WPF.Regions.Behaviors
{
    ...
    public class RegionActiveAwareBehavior : IRegionBehavior
    {
        ...
        /// </summary>
        public const string KEY = "ActiveAware";
        ...
        void OnCollectionChanged(object sender, NotifyCollectionChangedEventArgs e)
        {
            if (e.Action == NotifyCollectionChangedAction.Add)
            {
                foreach (object item in e.NewItems)
                {
                    ...
                    MVVMHelper.ViewAndViewModelAction(item, (Action<IActiveAware>)invocation);
            ...
            else if (e.Action == NotifyCollectionChangedAction.Remove)
            {
                ...
                {
                    static void invocation(IActiveAware activeAware) => activeAware.Active = false;
                    MVVMHelper.ViewAndViewModelAction(item, (Action<IActiveAware>)invocation);
                    InvokeOnSynchronizedActiveAwareChildren(item, invocation);
                }
            }
        ...
        void InvokeOnSynchronizedActiveAwareChildren(object item, Action<IActiveAware> invocation)
        {
            if (item is DependencyObject dependencyObjectView)
            {
                ...
                {
                    MVVMHelper.ViewAndViewModelAction(syncActiveView, invocation);
                }
            }
        }

        bool ShouldSyncActiveState(object view)
        {
