...
{
    /// <summary>
    /// The <see cref="BaseModuleCatalog"/> holds information about the modules that can be used by the
    ...
    ///
    /// It also verifies that the <see cref="BaseModuleCatalog"/> is internally valid. That means that
    /// it does not have:
    /// <list>
    ...
    ///     </item>
    /// </list>
    /// The <see cref="BaseModuleCatalog"/> also serves as a baseclass for more specialized Catalogs .
    /// </summary>
    public class BaseModuleCatalog : IModuleCatalog
    {
        readonly ModuleCatalogItemCollection _items;
        bool _loaded;

        /// <summary>
        /// Initializes a new instance of the <see cref="IModuleCatalog"/> class.
        /// </summary>
        public BaseModuleCatalog()
        {
        ...
        public BaseModuleCatalog(IEnumerable<IModuleInfo> modules)
            : this()
        {
        ...
        public IEnumerable<IModuleInfoGroup> Groups
            => Items.OfType<IModuleInfoGroup>();

        /// <summary>
        /// Gets or sets a value that remembers whether the <see cref="BaseModuleCatalog"/> has been validated already.
        /// </summary>
        ...
        protected IEnumerable<IModuleInfo> GrouplessModules
            => Items.OfType<IModuleInfo>();
        ...
        /// </summary>
        /// <exception cref="ModularityException">When validation of the <see cref="BaseModuleCatalog"/> fails, because this method calls <see cref="Validate"/>.</exception>
                ...
                if (moduleInfo.DependsOn != null && moduleInfo.DependsOn.Except(moduleNames).Any())
                {
                    throw new ModularityException(
                        ...
                        string.Format(
                            ...
                            "Module {0} depends on other modules that don't belong to the same group.",
                            moduleInfo.ModuleName));
                }
            }
        }
        ...
        protected virtual void ValidateUniqueModules()
        {
            ...
            {
                throw new DuplicateModuleException(duplicateModule, string.Format(
                    CultureInfo.CurrentCulture,
                    "A duplicated module with name {0} has been found by the loader.",
                    duplicateModule));
            }
        }
        ...
        protected virtual void ValidateDependenciesInitializationMode()
        {
            ...
            if (moduleInfo != null)
            {
                throw new ModularityException(
                    ...
                    string.Format(
                        CultureInfo.CurrentCulture,
                        "Module {0} is marked for automatic initialization when the application starts, but it depends on modules that are marked as OnDemand initialization. To fix this error, mark the dependency modules for InitializationMode=WhenAvailable, or remove this validation by extending the ModuleCatalog class.",
                        moduleInfo.ModuleName));
            }
        }
        ...
        class ModuleCatalogItemCollection : Collection<IModuleCatalogItem>, INotifyCollectionChanged
        {
